...
def setup_logging(log_dir, logger_type, logger_name=__name__, log_level=logging.INFO, mode=1):
    ...
    if logger_type == 'basic':
        ...
        console_handler.setFormatter(CustomFormatter())  # CustomFormatter()
...
def setup_device(device, lvl2, publvl3, data_keys):
    global printcolor, deviceD, MQTT_PUB_LVL1
...
def main():
    ...
    #t = Timer()
    # can use t.start() and t.stop() for quick timing numbers.  ina219 read is taking >3ms
    try:
        while True:
